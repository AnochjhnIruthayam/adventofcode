...
class DayTwo:
    def __init__(self, spreadsheet):
        self.spreadsheet = np.loadtxt(spreadsheet)

    def partOne(self):
        checksum = 0
        for row in range(np.array(self.spreadsheet).shape[0]):
            checksum += (max(self.spreadsheet[row])-min(self.spreadsheet[row]))
        return int(checksum)

    def partTwo(self):
        checksum = 0
        for row in range(self.spreadsheet.shape[0]):
            for i in range (self.spreadsheet[row].size):
                for j in range (self.spreadsheet[row].size):
                    if i == j:
                        continue
                    if self.spreadsheet[row][i] % self.spreadsheet[row][j] == 0:
                        checksum += self.spreadsheet[row][i] / self.spreadsheet[row][j]
                        break
        return int(checksum)


dayTwo = DayTwo(numbers)
